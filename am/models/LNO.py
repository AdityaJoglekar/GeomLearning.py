...
class LNO(torch.nn.Module):
    ...
    def forward(self, data):
        #indexing of x to get pos and phys is for cylinder flow
        x = data.x.unsqueeze(0) # space [B, N, C]
        # print('x',x.shape)
        pos = x[:,:,7:9] 
        # phys = torch.cat((x[:,:,0:7],x[:,:,9:]),dim =-1)
        
        if hasattr(data, 't'):
            t = data.t.unsqueeze(0)
            x = torch.cat((x, t), -1)
        if hasattr(data, 'dt'):
            d = data.dt.unsqueeze(0)
            x = torch.cat((x, d), -1)

        x1 = self.trunk_projector(pos)
        y = self.branch_projector(x)

        score = self.attention_projector(x1)
